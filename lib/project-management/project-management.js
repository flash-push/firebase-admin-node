...
exports.ProjectManagement = exports.ProjectState = void 0;
...
(function (ProjectState) {
    ProjectState["STATE_UNSPECIFIED"] = "STATE_UNSPECIFIED";
...
class ProjectManagement {
    ...
    getMetadata() {
        return this.getResourceName()
            ...
            .then((responseData) => {
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonNullObject(responseData), responseData, "getProjectMetadata()'s responseData must be a non-null object.");
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonEmptyString(responseData.projectId), responseData, '"responseData.projectId" field must be present in getProjectMetadata()\'s response data.');
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonEmptyString(responseData.projectNumber), responseData, '"responseData.projectNumber" field must be present in getProjectMetadata()\'s response data.');
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonEmptyString(responseData.name), responseData, '"responseData.name" field must be present in getProjectMetadata()\'s response data.');
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonEmptyString(responseData.state), responseData, '"responseData.state" field must be present in getProjectMetadata()\'s response data.');
            return {
                projectId: responseData.projectId,
                projectNumber: responseData.projectNumber,
                displayName: responseData.displayName,
                name: responseData.name,
                state: ProjectState[responseData.state] || ProjectState.STATE_UNSPECIFIED,
            };
        });
    }
    /**
     ...
     */
    listWebApps() {
        return this.listPlatformApps('web', 'listWebApps()');
    }
    /**
     ...
     */
    createWebApp(displayName) {
        return this.getResourceName()
            .then((resourceName) => {
            return this.requestHandler.createWebApp(resourceName, displayName);
        })
            .then((responseData) => {
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonNullObject(responseData), responseData, "createWebApp()'s responseData must be a non-null object.");
            (0, project_management_api_request_internal_1.assertServerResponse)(validator.isNonEmptyString(responseData.appId), responseData, '"responseData.appId" field must be present in createWebApp()\'s response data.');
            return new web_app_1.WebApp(responseData.appId, this.requestHandler);
        });
    }
    /**
    ...
    transformResponseToAppMetadata(responseData, projectId) {
        ...
        return responseData.apps.map((appJson) => {
            ...
            const metadata = {
                appId: appJson.appId,
                platform: app_metadata_1.AppPlatform[appJson.platform] ||
                    app_metadata_1.AppPlatform.PLATFORM_UNKNOWN,
                projectId,
        ...
        return this.getResourceName()
            ...
            return responseData.apps.map((appJson) => {
                ...
                if (platform === 'android') {
                    ...
                    return new web_app_1.WebApp(appJson.appId, this.requestHandler);
                }
            });
        });
    }
