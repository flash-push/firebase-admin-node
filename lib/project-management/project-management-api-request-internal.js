...
class ProjectManagementRequestHandler {
    ...
    /**
     * @param parentResourceName - Fully-qualified resource name of the project whose metadata
     *     you want to retrieve.
     */
    getProjectMetadata(parentResourceName) {
    ...
    /**
     * @param parentResourceName - Fully-qualified resource name of the project whose web apps
     *     you want to list.
     */
    listWebApps(parentResourceName) {
        return this.invokeRequestHandler('GET', `${parentResourceName}/webApps?page_size=${LIST_APPS_MAX_PAGE_SIZE}`, 
        /* requestData */ null, 'v1beta1');
    }
    /**
     * @param parentResourceName - Fully-qualified resource name of the project whose iOS apps
    ...
    /**
     * @param parentResourceName - Fully-qualified resource name of the project that you want
     *     to create the Web app within.
     */
    createWebApp(parentResourceName, displayName) {
        const requestData = {};
        if (validator.isNonEmptyString(displayName)) {
            requestData.displayName = displayName;
        }
        return this.invokeRequestHandler('POST', `${parentResourceName}/webApps`, requestData, 'v1beta1').then((responseData) => {
            assertServerResponse(validator.isNonNullObject(responseData), responseData, "createWebApp's responseData must be a non-null object.");
            assertServerResponse(validator.isNonEmptyString(responseData.name), responseData, "createWebApp's responseData.name must be a non-empty string.");
            return this.pollRemoteOperationWithExponentialBackoff(responseData.name);
        });
    }
    /**
    ...
    /**
     * Sets a field of a resource.
     *
     * @param resourceName - Fully-qualified resource name of the entity whose field you want to set.
     * @param field - The field to set.
     * @param value - The value to set.
     */
    setField(resourceName, field, value) {
        if (!validator.isNonEmptyString(field)) {
            throw new error_1.FirebaseProjectManagementError('invalid-argument', 'field must be a non-empty string.');
        }
        return this.invokeRequestHandler('PATCH', `${resourceName}?update_mask=${field}`, { [field]: value }, 'v1beta1').then(() => undefined);
    }
    /**
    ...
    deleteResource(resourceName) {
        ...
        /* requestData */ null, 'v1beta1').then(() => undefined);
    }
    pollRemoteOperationWithExponentialBackoff(operationResourceName) {
        ...
        return poller.poll(() => {
            ...
            /* requestData */ null).then((responseData) => {
